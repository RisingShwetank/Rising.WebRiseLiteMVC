...
namespace Rising.WebRise.Controllers
{
    public class ProcessController : Controller
    {
            ...
            try
            {
                ...
                if (status == true)
                {
                    ...
                    else
                    {
                        ...
                        ds = null;
                        qry = "INSERT INTO " + dbname + ".CUPOSITION Select  POSITIONDATE PDATE,SEGMENT_INDICATOR SEG_IND, SETTLEMENT_TYPE SETT_TYPE, CLEARING_MEMBER_CODE CMCODE, MEMBER_TYPE MEMTYPE, TRADING_MEMBER_CODE TMCODE, ACCOUNT_TYPE ACTYPE, '' CLIENTCD, INSTRUMENT_TYPE INSTRUMENT_TYPE, SYMBOL SYMBOL, EXPIRY_DATE EXPIRY_DATE, INSTRUMENT_TYPE || SYMBOL || to_char(EXPIRY_DATE,'DDMONYYYY') CONTNAME, STRIKE_PRICE STRIKE_PRICE, decode(OPTION_TYPE,'FF','FX',OPTION_TYPE) OPTIONTYPE, CA_LEVEL CALEVEL, BROUGHT_FORWARD_LONG_QUANTITY BFLONGQTY, BROUGHT_FORWARD_LONG_VALUE BFLONGVAL, BROUGHT_FORWARD_SHORT_QUANTITY BFSHORTQTY, BROUGHT_FORWARD_SHORT_VALUE BFSHORTVAL, DAY_BUY_OPEN_QUANTITY BOPEN_QTY, DAY_BUY_OPEN_VALUE BOPEN_VAL, DAY_SELL_OPEN_QUANTITY SOPEN_QTY, DAY_SELL_OPEN_VALUE SOPEN_VAL, PRE_EXASSGN_LONG_QUANTITY PREEXAS_LONGQTY, PRE_EXASSGN_LONG_VALUE PREEXAS_LONGVAL, PRE_EXASSGN_SHORT_QUANTITY PREEXAS_SHORTQTY, PRE_EXASSGN_SHORT_VALUE  PREEXAS_SHORTVAL, EXERCISED_QUANTITY EXQTY, ASSIGNED_QUANTITY ASQTY, POST_EXASSGN_LONG_QUANTITY POSTEXAS_LONGQTY, POST_EXASSGN_LONG_VALUE POSTEXAS_LONGVAL, POST_EXASSGN_SHORT_QUANTITY POSTEXAS_SHORTQTY, POST_EXASSGN_SHORT_VALUE POSTEXAS_SHORTVAL, SETTLEMENT_PRICE SETTPRICE, NET_PREMIUM NETPREM, DAILY_MTM_SETTLEMENT_VALUE MTMSETT_VAL, FUTURES_FINAL_SETTLEMENT_VALUE FUTFINSETT_VAL, EXERCISEDASSIGNED_VALUE EXASVAL,'' CLOSPRICE,'" + model.Exchange + "'EXCHANGE,'" + model.SessionId + "' SESSIONID,'' SETTNO, CLIENT_ACCOUNTCODE ORGCODE from " + dbname + ". NSE_POSITION_TEMP_TABLE";
                        ds = MvcApplication.OracleDBHelperCore().CustomHelper.ExecuteDataSet(qry, Session["SelectedConn"].ToString());
                        qry = "SELECT COUNT(*) FROM " + dbname + ".CUPOSITION WHERE PDATE = to_date('" + model.Date.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and Exchange = '" + model.Exchange + "' and SessionId = '" + model.SessionId + "'";
                        ...
                        if (model.ImportBasis == "Actual Code")
                        {
                            qry = "Update IFSC.CUPOSITION A set clientcd = (select party_cd from ifsc.cupartymst_fixes B where B.shortcode = A.ORGCODE and A.exchange = B.exchange and A.exchange = 'NSE' and PDATE = to_date('" + model.Date.ToString("dd-MM-yyyy") + "', 'dd-MM-yyyy') and sessionid = '1')where PDATE = to_date('" + model.Date.ToString("dd-MM-yyyy") + "', 'dd-MM-yyyy') and sessionid = '1' and exchange = 'NSE'";
                            ds = MvcApplication.OracleDBHelperCore().CustomHelper.ExecuteDataSet(qry, Session["SelectedConn"].ToString());
                        }
                        else
                        {
                            qry = "Update IFSC.CUPOSITION A set clientcd = (select shortcode from ifsc.cupartymst_fixes B where B.shortcode = A.ORGCODE and A.exchange = B.exchange and A.exchange = 'NSE' and PDATE = to_date('" + model.Date.ToString("dd-MM-yyyy") + "', 'dd-MM-yyyy') and sessionid = '1')where PDATE = to_date('" + model.Date.ToString("dd-MM-yyyy") + "', 'dd-MM-yyyy') and sessionid = '1' and exchange = 'NSE'";
                            ds = MvcApplication.OracleDBHelperCore().CustomHelper.ExecuteDataSet(qry, Session["SelectedConn"].ToString());
                        }
                    }
        ...
        public ActionResult PositionFileOverrideConfirmation(DateTime TradeDate, string Exchange, string ImportBasis, string SessionId, bool IsOverrideConfirmed, bool IsDeleteConfirmed)
        {
            ...
            if (IsOverrideConfirmed)
            {
                ...
                //int insertedRowCount = Convert.ToInt32(MvcApplication.OracleDBHelperCore().CustomHelper.ExecuteDataSet(qry, Session["SelectedConn"].ToString()));
                if (ImportBasis == "Actual Code")
                {
                    qry = "Update IFSC.CUPOSITION A set clientcd = (select party_cd from ifsc.cupartymst_fixes B where B.shortcode = A.ORGCODE and A.exchange = B.exchange and A.exchange = 'NSE' and PDATE =to_date('" + TradeDate.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and sessionid = '1')where PDATE =to_date('" + TradeDate.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and sessionid = '1' and exchange = 'NSE'";
                    ds = MvcApplication.OracleDBHelperCore().CustomHelper.ExecuteDataSet(qry, Session["SelectedConn"].ToString());
                }
                else
                {
                    qry = "Update IFSC.CUPOSITION A set clientcd = (select shortcode from ifsc.cupartymst_fixes B where B.shortcode = A.ORGCODE and A.exchange = B.exchange and A.exchange = 'NSE' and PDATE =to_date('" + TradeDate.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and sessionid = '1')where PDATE =to_date('" + TradeDate.ToString("dd-MM-yyyy") + "','dd-MM-yyyy') and sessionid = '1' and exchange = 'NSE'";
                    ds = MvcApplication.OracleDBHelperCore().CustomHelper.ExecuteDataSet(qry, Session["SelectedConn"].ToString());
                }
            }
